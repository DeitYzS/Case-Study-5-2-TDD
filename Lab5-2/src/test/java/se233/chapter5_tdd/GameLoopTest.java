...
public class GameLoopTest {
    ...
    @Before
    public void unit() throws NoSuchMethodException {
    ...
    @Test
    public void checkedScore() throws Exception {
        gameLoopUnderTest = new GameLoop(new Platform(), new Snake(new Point2D(0, 0)), new Food(new Point2D(0,2)));
        gameLoopUnderTest.getPlatform().setKey(KeyCode.DOWN);
        clockTickHelper();
        clockTickHelper();
        assertTrue(gameLoopUnderTest.getSnake().getScore() > 0);
    }

    @Test
    public void checkedSpecialFoodScore() throws Exception {
        gameLoopUnderTest = new GameLoop(new Platform(), new Snake(new Point2D(0, 0)), new Food(new Point2D(0,2)));
        gameLoopUnderTest.getFood().isSpecial();
        gameLoopUnderTest.getFood().setSpecial(true);
        gameLoopUnderTest.getPlatform().render(new Snake(new Point2D(0, 0 )), new Food(new Point2D(0, 2)));
        gameLoopUnderTest.getFood().setPosition(new Point2D(0, 2));
        gameLoopUnderTest.getPlatform().setKey(KeyCode.DOWN);
        clockTickHelper();
        clockTickHelper();
        assertTrue(gameLoopUnderTest.getSnake().getScore() > 4);
    }
}
